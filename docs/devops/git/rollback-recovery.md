...
不确定提交是否已推送时，先执行 `git branch -vv` 和 `git status --short --branch`。必要时执行 `git fetch <remote>`，再比较本地分支与 upstream 的提交；本地的远程跟踪引用不是实时远端状态，未 fetch 前不能据此断言远端没有变化。
...
| 未推送提交 | 本地分支领先 upstream，且提交尚未共享 | `git commit --amend` 或 `git reset` | 是 | 改写本地分支历史；共享前才适用。 |
...
**操作前检查：** 使用 `git log --oneline --decorate -n 3` 确认目标是最近一次提交，并用 `git branch -vv` 确认该提交尚未共享。需要补充文件内容时，先检查 `git diff` 和 `git diff --staged`。
...
**操作前检查：** 使用 `git log --oneline --decorate -n 10` 确认要回退到的 `<commit>`，再用 `git branch -vv` 确认待撤销提交尚未共享。先执行 `git branch backup/<name>` 固定当前位置，避免选错目标后无引用可查。
...
**执行：** 使用 `git revert <commit>` 创建一个新的提交来反向应用 `<commit>` 的改动。`revert` 不等同于删除历史：原提交和撤销提交都会保留在历史中。若 Git 报告冲突，按[冲突解决](conflict-resolution.md)处理文件后使用 `git revert --continue`；不再继续本次撤销时使用 `git revert --abort` 回到开始前状态。
...
- 需要保留一个可回退的提交位置时，使用 `git branch backup/<name>`；它不会保存当前未提交的工作区内容。需要临时保存已跟踪和未跟踪改动时，使用 `git stash push -u`；它不是长期备份，也不会替代已推送的共享历史。需要保存可审阅的未提交差异时，使用 `git diff > <patch-file>`；它只包含当前未暂存差异，不包含暂存区差异或未跟踪文件。
...
| 补充或改写最近的未共享提交 | `git commit --amend` | 改写最近提交；先确认未共享。 |
| 撤销未共享提交并继续编辑内容 | `git reset --soft <commit>` 或 `git reset --mixed <commit>` | 改写本地历史；前者保留暂存，后者保留工作区并取消暂存。 |
...
| 不确定提交是否已推送 | `git branch -vv`、`git status --short --branch`，必要时 `git fetch <remote>` | 不要把未更新的远程跟踪引用当作实时远端，也不要贸然改写历史。 |
